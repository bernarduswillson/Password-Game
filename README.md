...
Dalam tugas besar ini, Anda diminta untuk membuat sebuah game yang mirip dengan “Password Game” berbentuk website. Anda harus mengerjakan semua poin fitur wajib (8) dan minimal 2 fitur lanjutan. Nilai anda akan dikenakan penalti jika anda mengerjakan kurang dari 10 fitur tersebut.
...
3. Berikut adalah daftar rule yang harus diimplementasikan (harus secara urut):
    ...
    - Rule 10 – Your password must include this CAPTCHA <br>
    ...
    - Rule 11 – Your password must include a leap year
    - Rule 12 – 🥚 This is my chicken Paul. He hasn’t hatched yet. Please put him in your password and keep him safe <br>
    (Pastikan emoji telur tidak terhapus. Kalau terhapus, pemain dinyatakan kalah)
    - Rule 13 – A sacrifice must be made. Pick <b><i>X</i></b> letters that you will no longer be able to use
    - Rule 14 – Oh no! Your password is on fire 🔥. Quick, put it out! <br>
    (Emoji api akan “membakar” 1 huruf (menghapus huruf dan menggantikannya dengan emoji api) setiap <b><i>X</i></b> detik dimulai dari huruf terakhir. Api tidak akan berhenti membakar sampai semua emoji api pada textfield sudah dihapus)
...
4. Pemain dapat dinyatakan kalah jika pemain sudah tidak memiliki satupun opsi jawaban atau tidak ada jawaban yang dapat memenuhi ketentuan pada current rule, algoritma pencarian opsi dibebaskan. Selain itu, kondisi kalah juga dapat terjadi pada kasus-kasus unik seperti rule 12 dan rule 16. Sedangkan, pemain dapat dinyatakan menang jika pemain telah memenuhi semua rule (20 rule).

5. Program harus memiliki perintah cheat untuk meng-skip current rule. Namun, anda juga harus memberikan salah satu opsi jawaban agar current rule terpenuhi. Anda dapat menggunakan command tersebut dengan mengetik “cheat” pada textfield, kemudian akan secara otomatis menghapus dan mengganti kata-kata “cheat” tersebut menjadi salah satu opsi jawaban yang tersedia. Jawaban harus melanjutkan password yang telah dibuat pemain pada rule sebelumnya, tidak boleh membuat password dari awal. Namun, jika ternyata sudah tidak ada opsi jawaban yang tersedia, anda diperbolehkan untuk mengganti password dari rule-rule sebelumnya dari rule yang paling tinggi. Misal: rule 17 tidak ada opsi jawaban, maka anda boleh mengganti password pada rule 16, dan jika masih tidak ada opsi jawaban, anda boleh mengganti password pada rule 15. Untuk metode dan algoritma pencarian jawaban dibebaskan.
...
5. Penggunaan docker dan deployment aplikasi (backend dan frontend) sehingga dapat diakses dan dimainkan pada web. Docker hanya berfungsi dan digunakan saat local development saja, tidak wajib digunakan pada saat deployment.

6. Membuat global leaderboard. Global leaderboard akan berisi highest score setiap pemain yang berbeda. 1 device/client dapat dianggap sebagai 1 pemain.

7. Membuat sebuah mobile app. Fitur ini dihitung sebanyak 3 fitur lanjutan. Jika sudah mengerjakan fitur ini, tidak perlu lagi mengerjakan versi websitenya.

8. Membuat fitur multiplayer atau 1v1 dengan player lain. Mekanismenya dibebaskan dengan bare minimum setiap pemain dapat melihat progress dan score lawannya secara real-time saat sesi permainan sedang berjalan.  Fitur ini dihitung sebanyak 5 fitur lanjutan.

9. Fitur-fitur dan ide-ide lainnya yang dianggap keren dan kreatif dapat menjadi konsiderasi untuk dijadikan fitur lanjutan. Aku tunggu kreativitas kalian!
...
**Semangat! Goodluck! Jangan lupa tidur.**